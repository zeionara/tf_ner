...
def make_vocabs(root_folder):
    # 1. Words
    # Get Counter of words on all the data, filter by min count, save
    def words(name):
        return f'{root_folder}/{name}.words.txt'
    ...
    with Path(f'{root_folder}/vocab.words.txt').open('w') as f:
    ...
    with Path(f'{root_folder}/vocab.chars.txt').open('w') as f:
    ...
    def tags(name):
        return f'{root_folder}/{name}.tags.txt'
    ...
    with Path(f'{root_folder}/vocab.tags.txt').open('w') as f:
